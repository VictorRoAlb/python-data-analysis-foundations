...
Public Python rewrite of a first-semester data-analysis coursework block originally developed in R.
...
This public version is built around two translated case studies:
...
The original coursework was written in R Markdown. For the portfolio, the logic has been reworked into Python scripts so the public repository fits better with the rest of the profile.
...
- `src/build_summary_panel.py`
  Small utility that combines the main public-safe figures into a single portfolio panel.
- `figures/`
  Curated output figures used in the README and on the portfolio website.
- `docs/project_notes.md`
  Notes on what was translated and what was intentionally left out of the public version.
- `notebooks/README.md`
...
- the public repository keeps the code readable and reproducible without shipping the original local coursework data files.

## How to regenerate the public figures

The repository does not publish the original coursework datasets. To regenerate the figures locally, provide the paths through environment variables:
...
The original source files used in the course are not redistributed here. The public repository keeps:
